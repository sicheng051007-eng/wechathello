...
鼓励语在 [`love_push/compose.py`](love_push/compose.py) 的 `MORNING_WORDS` 和 `EVENING_WORDS` 中，可以直接替换为你们之间更有意义的话。模板消息中的彩色 Emoji 在部分安卓客户端会显示为方框；项目会自动把常见 Emoji 降级为 `♥`、`☀`、`☁`、`☂`、`☺`、`★`、`☾` 等兼容图案，未知的高位 Emoji 会在发送前移除。
